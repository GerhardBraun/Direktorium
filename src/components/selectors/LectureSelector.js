...
const LectureSelector = ({
    ...
    const availableAlternatives = useMemo(() => {
        ...
        // Standard-Button (immer Index -1 intern, aber 0 für UI)
        if (firstKeyword && firstAlternatives.length > 0) {
            firstButtons.push({
                index: -1, // Spezieller Index für Standard
                uiIndex: 0,
                label: 'Standard',
                button: chain(getValue('les_buch'), getValue('les_stelle')),
                resp: abbreviate(getValue('resp1')),
        ...
        firstAlternatives.forEach((alt, index) => {
            ...
            firstButtons.push({
                index: index, // Echter Array-Index
                uiIndex: index + 1,
                label: index === 0 ? 'Alternative' : `Alternative ${index + 1}`,
                button: chain(
                    checkLanguageField('les_buch', altData),
                    checkLanguageField('les_stelle', altData)
                ),
                resp: abbreviate(checkLanguageField('resp1', altData)),
        ...
        if (secondKeyword && secondAlternatives.length > 0) {
            secondButtons.push({
                ...
                uiIndex: 0,
                label: 'Standard',
                button: chain(getValue('patr_autor'), getValue('patr_werk'), ':'),
                resp: abbreviate(getValue('patr_resp1')),
        ...
        secondAlternatives.forEach((alt, index) => {
            ...
            secondButtons.push({
                ...
                resp: abbreviate(checkLanguageField('patr_resp1', altData)),
        ...
        return {
            first: {
                ...
                hasAlternatives: firstButtons.length > 1,
                hasAlternativeText: firstButtons.some(b => b.index !== -1 && b.hasText), // Nur echte Alternativen mit Text
                onlyAlternativeResp: firstButtons.some(b => b.index !== -1 && b.onlyResp) // Nur echte Alternativen mit nur Resp
            },
            second: {
                keyword: secondKeyword,
                alternatives: secondAlternatives,
                buttons: secondButtons,
                hasAlternatives: secondButtons.length > 1,
                hasAlternativeText: secondButtons.some(b => b.index !== -1 && b.hasText), // Nur echte Alternativen mit Text
                onlyAlternativeResp: secondButtons.some(b => b.index !== -1 && b.onlyResp) // Nur echte Alternativen mit nur Resp
            }
        };
    ...
    const selected = (field) => {
        ...
        if (selectedIndex === -1 || !availableAlternatives[lectureType].hasAlternatives) {
            return getValue(field);
        }

        // Alternative aus Datenbank verwenden
        const alternatives = availableAlternatives[lectureType].alternatives;
        if (!alternatives[selectedIndex]) return getValue(field);

        const alternativeData = alternatives[selectedIndex][lectureType];
                ...
                {buttons.map((button) => (
                    ...
                    >
                        <div className="flex items-baseline gap-0">
                            ...
                            <div>
                                {lectureMeta.onlyAlternativeResp ? button.resp : button.button}
                            </div>
                        </div>
                    </button>
                ))}
    ...
    useEffect(() => {
        setSelectedLecture(prev => ({
            first: availableAlternatives.first.hasAlternatives ?
                (prev.first >= availableAlternatives.first.buttons.length - 1 ? -1 : prev.first) : -1,
            second: availableAlternatives.second.hasAlternatives ?
                (prev.second >= availableAlternatives.second.buttons.length - 1 ? -1 : prev.second) : -1
        }));
