...
13. **`excludeYear` deckt zwei unabhängige Zyklen ab** (Stand 21.07.2026): den dreijährigen Sonntagszyklus (`a`/`b`/`c`, kleingeschrieben, Vergleich gegen `finalData.yearABC`) und den zweijährigen Lesehore-Werktagszyklus (`I`/`II`, großgeschrieben, Vergleich gegen `finalData.yearI2`) – bewusst unterschiedliche Schreibweise, damit in Excel auf einen Blick erkennbar ist, welcher Zyklus gemeint ist. Beide werden **ausschließlich** in `getPrayerTexts` aus derselben `lectureYear`-Berechnung abgeleitet, die dort auch über `lectureData = lectureYear % 2 === 0 ? lecture2Data : null` entscheidet, ob `Lecture2.ts` geladen wird (inkl. Advents-Verschiebung) – so bleibt „welches Lesejahr lädt" und „welche Alternative wird gefiltert" garantiert konsistent. Beide fließen über `getPrayerTexts`' Rückgabe und `...todayData` automatisch in `finalData` (die frühere doppelte, eigenständige `yearABC`-Neuberechnung in `processBrevierData` mit verschachtelten Bedingungen wurde entfernt). Der Filter in `resolveLectureSelectorsForSource` prüft `yearABC` und `yearI2` gleichwertig; keine Sonderbehandlung nach Hore nötig, da `I`/`II` in der Praxis nur bei Lesehore-Alternativen vorkommt.
14. **Messlesungs-Alternativen (Stand 03.08.2026, Datenebene fertig, UI noch offen)**: Die Alternativen-Auflösung ist auf die fünf Messlesungs-Felder verallgemeinert – `Typ`-Spalte in Excel akzeptiert jetzt zusätzlich zu `les`/`patr` auch `ms_les`, `ms_les2`, `ms_ev`, `ms_aps`, `ms_ruf`, jeweils mit eigenem `ms_<typ>_selector`-Array (`ALTERNATIVES_TYPE_FIELDS` in converter.py, dieselbe Schleife wie `les_text`/`patr_text` in `resolveLectureSelectorsForSource`). `ms_aps`/`ms_ruf` kennen nur `stelle`/`text` (kein `buch`/`motto`, siehe LectureABC.ts). Perikopen-Abgleich läuft danach gegen `Perikopen_Messe.ts` (`applyPerikopeMesse`, analog `applyPerikope`) – wenn ein `ms_<typ>_selector`-Array existiert, auf dessen Einträge, sonst wie bisher auf das Textfeld direkt. `Motto` ist noch keine Spalte im Excel-Blatt „Alternativen" (nur in „Perikopen_Messe"); fehlt sie, bleibt das Feld leer, kein Fehler. **Bewusst nicht enthalten**: Die REF&-Marker-Logik (Punkt 11) gilt nicht für Messlesungen, da sie keine diözesane Referenz über `Laudes.referenz` kennen. **Offen**: `MassReadings.js` liest die `ms_*_selector`-Arrays noch nicht (kein Auswahl-UI) – eigenes, größeres Folgeprojekt.
15. **`excludeYear` erlaubt Buchstaben-Kombinationen beim dreijährigen Zyklus** (Stand 03.08.2026): `"bc"` schließt sowohl B als auch C aus, die Lesung erscheint dann nur im Lesejahr A. Umgesetzt über `entry.excludeYear.includes(yearABC)` statt `===` (yearABC ist immer genau ein Buchstabe, daher matcht `.includes()` sowohl einzelne Buchstaben als auch Kombinationen). Gilt nur für den a/b/c-Zyklus; beim zweijährigen `yearI2`-Zyklus bleibt es bei `===`, da `I`/`II` sich schon gegenseitig ausschließen und Kombinationen dort keinen Sinn ergäben.
