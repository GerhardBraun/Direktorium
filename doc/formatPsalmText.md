...
formatPrayerText erkennt: Wenn marker='cant', dann soll auch die zusätzliche Funktion `formatCantMarkers` zur Umsetzung der |- und Countdown-Marker in Unterstreichungen und Klammern aufgerufen werden. Der Parameter hour wird dann in die Konstante `mode` übernommen und hour='' gesetzt, damit es keine Fehler oder anderen unerwünschten Effekte gibt.
...
## 1.1 In der Zeile gibt es einen |-Marker:
...
Die Countdown-Marker spielen dann keine Rolle, außer zur Erkennung eines männlichen Versschlusses anhand des 4-Markers.
...
## 3.1 Kadenzbeginn
liegt bei der Silbe mit dem Countdown-Marker c=v;
Ausnahme: Wenn es einen `4`-Marker gibt, liegt der Kadenzbeginn bei c=v+1
*Diese `4`-Marker steht in Versen mit Schlusskadenz und männlichem Versschluss.*

## 3.2 Klammern
Wenn auf die letzte betonte Silbe (`||`) mehrere unbetonte Silben folgen, werden diese durch eine Klammer verbunden.
**Sonderfall**: Wenn auf die Silbe mit dem einfachen `|`-Marker erst *nach* der Silbe mit dem doppelten `||`-Marker steht, werden die Silben von `||` einschließlich bis `|` ausschließlich mit einer Klammer verbunden und alle Silben ab `|` bis zum Versschluss.

# zusätzliche 0-Marker
Zur Vereinfachung der Arbeit von formatCantMarkers werden alle Silbengrenzen innerhalb der Kadenz, die nicht schon durch |- oder ~- oder Countdown-Marker erkennbar sind, durch `0`-Marker gekennzeichnet.

# Verschachtelung von ^u- und ^b-Marker
...
Das muss bei der Segmentierung und Auflösung der Marker in Unterstreichungen und Klammern berücksichtigt werden.
